...
export function Pricing({ dict, locale }: { dict: Dict["pricing"]; locale: Locale }) {
  return (
            ...
            return (
              ...
              >
                ...
                <span className="inline-flex w-fit items-center rounded-full bg-[#F1F5F9] px-3 py-1 text-[11.5px] font-semibold uppercase tracking-[0.08em] text-[#475569]">
                  {dict.sectorTags[pack.id]}
                </span>
                <h3 className="mt-4 text-[24px] font-bold tracking-[-0.02em] text-[#0B1220]">{pack.name[locale]}</h3>
                <p className="mt-1.5 text-[14px] leading-[1.55] text-[#64748B]">{pack.tagline[locale]}</p>
                ...
                <div className="mt-2 text-[13px] text-[#64748B]">{dict.paymentNote}</div>
                ...
                <div className="mt-8 text-[13px] text-[#64748B]">{pack.delivery[locale]}</div>

                <Link
                  href={`${path("contact", locale)}?pack=${CONTACT_SLUG[pack.id]}`}
                  className={[
                ...
                >
                  {dict.ctaPrefix} {pack.name[locale]}
